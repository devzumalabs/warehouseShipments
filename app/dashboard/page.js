...
const Page = () => {
  ...
  const calculateWorkTimeElapsed = (dateOrder) => {
    ...
    const [datePart, timePart] = dateOrder
      .split(",")
      .map((part) => part.trim());
    ...
    const orderDate = new Date(
      `${year}-${month}-${day}T${hours}:${minutes}:${seconds}-07:00`
    );
    const now = new Date();
    ...
    isAuthenticated && (
      <div className="w-full h-screen px-6 sm:px-3">
        <div className="container m-auto py-6">
          ...
          <h2 className="text-gray-700 text-2xl pt-10 pb-2 font-semibold">
            Historial de envíos
          </h2>
          <div
            className={`flex-col rounded-3xl py-3 px-12 bg-white overflow-scroll sm:overflow-hidden h-[540px] flex ${
              isLoading ? "justify-center items-center" : "justify-between"
            }`}
          >
            {isLoading ? (
                ...
                <div className="footer flex gap-2 justify-end pt-8 pb-4">
                  {Array.from({ length: totalPages }, (_, index) => (
                    <div
                      ...
                      className={`w-7 h-7 text-white flex justify-center items-center rounded-md ${
                        currentPage === index + 1
                          ? "bg-[#0072F6]"
                          : "bg-[#9ca3af]"
